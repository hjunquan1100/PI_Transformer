...
class Downstream_Dataset(Dataset):
    def __init__(self, dataset, tokenizer, max_token_len, descriptor_cols=None, sample_weights=None):
        self.tokenizer = tokenizer
        self.dataset = dataset
        self.max_token_len = max_token_len
        self.descriptor_cols = list(descriptor_cols) if descriptor_cols else []
        self.has_desc = len(self.descriptor_cols) > 0
        self.sample_weights = sample_weights
        if self.sample_weights is not None:
            sw = np.asarray(self.sample_weights, dtype=np.float64)
            if sw.shape[0] != len(self.dataset):
                raise ValueError("sample_weights length %s != dataset %s" % (sw.shape[0], len(self.dataset)))
    ...
    def __getitem__(self, i):
        ...
        if self.has_desc:
            desc = self.dataset.iloc[i][self.descriptor_cols].values.astype(np.float64)
            out["desc"] = torch.tensor(desc, dtype=torch.float64)
        if self.sample_weights is not None:
            out["weight"] = torch.tensor(float(self.sample_weights[i]), dtype=torch.float32)
        return out
...
class ContrastivePolymerPairDataset(Dataset):
    """label SMILES : itemspolymer DataAugmentation ,  NT-Xent / InfoNCE."""

    def __init__(self, smiles_list, tokenizer, blocksize, aug_indicator=4):
        self.smiles_list = [str(s) for s in smiles_list]
        self.tokenizer = tokenizer
        self.blocksize = blocksize
        self.aug = DataAugmentation(aug_indicator)

    def __len__(self):
        return len(self.smiles_list)

    def _encode(self, smi):
        enc = self.tokenizer(
            smi,
            add_special_tokens=True,
            max_length=self.blocksize,
    ...
    def __getitem__(self, i):
        ...
        arr = self.aug.generate_smiles(smi)
        flat = arr.flatten().tolist() if arr is not None and len(arr) > 0 else []
        if not flat or "None" in flat:
            s1, s2 = smi, smi
        else:
            s1 = str(arr[0, 0])
            j = 1 if len(arr) > 1 else 0
            s2 = str(arr[j, 0])
            if s2 == "None":
                s2 = s1
        a_ids, a_mask = self._encode(s1)
        b_ids, b_mask = self._encode(s2)
        return {
            "input_ids_1": a_ids,
            "attention_mask_1": a_mask,
            "input_ids_2": b_ids,
            "attention_mask_2": b_mask,
        }